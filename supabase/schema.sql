...
begin
  foreach t in array array[
    ...
    -- Webhook URL の同期先と、履歴のまとめ送信の進み具合 (このファイル末尾の節)。
    -- ここではまだ作られていないので初回は素通りするが、
    -- 末尾のそれぞれの節が自前で revoke している
    'discord_settings',
    'partner_backlog_sends'
  ]
  loop
...
-- ============================================================
-- 預かり金の履歴を「まとめて Discord に送る」ための進み具合 (機能009 の穴埋め)
--
-- 既存プロジェクトへの追加は supabase/migration-partner-backlog.sql を実行する。
-- 内容は同じもので、どちらも複数回実行して安全。
--
-- ---- なぜテーブルを1つ増やしたのか ----
