...
/**
 * お店として「同じ店」と見なすときのキー。前後の空白だけを落とした店名そのもの。
 *
 * **完全一致にしてある**(検索の表記ゆれ吸収 = normalizeSearchText は通さない)。
 * レポートのお店別集計 (report.ts の rankByStore) と、履歴のお店絞り込み
 * (historyFilter.ts の stores) は **必ずこの同じ関数を通すこと**。
 *
 * 揺れを吸収したほうが親切に見えるが、片方だけ緩めてはいけない。
 * レポートが「オカモトセルフ 6件」と出している行を押した先の履歴が7件になると、
 * 数字が食い違っていること自体が不具合に見える。
 * (「ｵｶﾓﾄｾﾙﾌ 東店」まで揺れごと拾いたいときは、フリーワード検索が既にその役目を持っている)
 * 変えるなら rankByStore と絞り込みを同時に、この関数の中で変えること。
 */
export function storeKey(t: { store: string }): string {
  return t.store.trim()
}
