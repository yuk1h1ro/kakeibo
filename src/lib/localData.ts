...
/**
 * ログアウトを押した **直後** に出す1段目の確認文。(純粋関数)
 *
 * このボタンは、目隠し・設定と並んでヘッダーに常時出ている。指が滑って
 * 当たることがあり、当たった瞬間にログアウトが済んでしまうと、次に出る
 * のが「端末のデータも消しますか?」になる。**押すつもりが無かった人に、
 * いきなり消去の可否を聞く** のがいちばん危ない並びなので、その手前で
 * 一度止める。
 *
 * ここでは「消すかどうか」をまだ聞かない。段を分ける意味が無くなるため。
 * 消す話は2段目 (signOutConfirmText) に置き、そこに進むことだけ予告する。
 *
 * @param pending 未同期の件数。0 なら触れない(無い話を読ませない)
 */
export function signOutIntentText(pending: number): string {
  const lines = ['ログアウトしますか?', '']
  if (pending > 0) {
    // まだこの端末にしかない記録。ログアウトしても消えはしないが、
    // 同期してからのほうが安全なので、押す前のここで知らせる
    lines.push(
      `未同期の記録が ${pending}件あります。オンラインに戻して同期してからのほうが安全です。`,
      ''
    )
  }
  lines.push(
    '・サーバー上の記録は1件も消えません',
    '・次に使うときは、もう一度 Google でログインします',
    '・この端末に残っているデータを消すかどうかは、このあと改めて確認します',
    '',
    'ログインし直すだけなら、ログアウトは要りません(有効期限はありません)。'
  )
  return lines.join('\n')
}

/**
 * ログアウトの後に出す2段目の確認文。何が消えるかを並べる。(純粋関数)
 * ここで OK を押して初めて、端末内のデータが消える。
 */
